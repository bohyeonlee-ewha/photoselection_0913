...
function normalizeActivity(value: unknown) {
  ...
  const aliases: Record<string, (typeof activities)[number]> = {
    ...
    "감각탐구": "감각·탐구영역", "탐구": "감각·탐구영역", "바깥": "실외놀이", "실외": "실외놀이",
  };
...
function findActivity(content: string) {
  const labels: Array<[string, (typeof activities)[number]]> = [
    ...
    ["감각·탐구", "감각·탐구영역"], ["감각탐구", "감각·탐구영역"], ["실외", "실외놀이"], ["바깥", "실외놀이"], ["기타", "기타"],
  ];
  ...
  const response = await fetch(`${baseUrl}/messages`, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: [
          { type: "text", text: `유아교육 사진을 보고 사진 속 실제 놀이 모습에 따라 아래 영역 중 하나로 분류하세요: ${activities.join(", ")}. 미술 재료·그리기·만들기는 미술놀이, 악기·노래·리듬은 음률, 역할극·병원놀이·가게놀이는 역할놀이, 책·이야기·글자는 언어영역, 숫자·퍼즐·블록·분류는 수·조작영역, 자연·실험·감각 탐색은 감각·탐구영역, 달리기·체육·신체 움직임은 신체활동, 실외에서 놀이하는 모습은 실외놀이입니다. 사진에 놀이 활동이 보이지 않는 단순 인물 사진이나 일반 사진은 기타로 분류하세요. 미분류는 사진을 열 수 없거나 판단할 정보가 전혀 없을 때만 선택하세요. 반드시 JSON 한 줄로만 답하세요: {"activity":"미술놀이","confidence":0.9,"reason":"판단 근거"}` },
